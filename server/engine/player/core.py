...
class Player(
    ...
    def die(self, world: Optional['World'] = None) -> None:
        ...
        if self.runtime_state.combat is not None:
            self.runtime_state.combat.in_combat = False
        ...
        if self.runtime_state.combat is not None:
            self.runtime_state.combat.targets.clear()
        self.active_effects.clear()
        
        local_world = self.world
        if local_world and self.runtime_state.magic is not None:
            all_summon_ids = [inst_id for ids in self.runtime_state.magic.summons.values() for inst_id in ids]
    ...
    def respawn(self) -> None:
        ...
        if self.runtime_state.magic is not None:
            self.runtime_state.magic.mana = self.runtime_state.magic.max_mana
            self.runtime_state.magic.cooldowns.clear()
            self.runtime_state.magic.summons = {}
        self.is_alive = True
        if self.runtime_state.combat is not None:
            self.runtime_state.combat.in_combat = False
