# engine/config/config_player.py
"""
Configuration for the player character, including defaults, stats, and leveling.
"""

# --- Player Defaults & Base Values ---
PLAYER_DEFAULT_NAME = "Player"
# The ability pool: what an ability spends. Called "mana" throughout this file
# and the player runtime because that is what the first content set named it;
# which resource it *is*, and what it is called on screen, comes from the
# content set's `resources` contract (`engine/contracts/resources.py`). These
# are the curve's numbers, not the resource's name.
ABILITY_POOL_BASE = 50
ABILITY_POOL_PER_STAT_POINT = 5
ABILITY_POOL_REGEN_BASE = 1.0
ABILITY_POOL_LEVEL_UP_MULTIPLIER = 1.15
ABILITY_POOL_LEVEL_UP_STAT_DIVISOR = 2
ABILITY_POOL_REGEN_STAT_DIVISOR = 20
PLAYER_DEFAULT_MAX_MANA = ABILITY_POOL_BASE
PLAYER_BASE_ATTACK_POWER = 5
PLAYER_BASE_DEFENSE = 3
PLAYER_BASE_ATTACK_COOLDOWN = 2.0
PLAYER_BASE_MANA_REGEN_RATE = ABILITY_POOL_REGEN_BASE
PLAYER_BASE_HEALTH_REGEN_RATE = 1.0
PLAYER_MAX_COMBAT_MESSAGES = 10
PLAYER_DEFAULT_MAX_TOTAL_SUMMONS = 100
# Neutral fallback for the player's starting class/role label, shown until a
# content set names its own via "player_defaults.player_class" in its
# ruleset (see fantasy_frontier's ruleset.json for a worked example).
DEFAULT_PLAYER_CLASS_NAME = "Player"

# --- Player Health Calculation ---
PLAYER_BASE_HEALTH = 80
PLAYER_CON_HEALTH_MULTIPLIER = 2.0
PLAYER_LEVEL_HEALTH_BASE_INCREASE = 5
PLAYER_LEVEL_CON_HEALTH_MULTIPLIER = 0.5

# --- Player Stats & Leveling ---
PLAYER_DEFAULT_STATS = {
    "strength": 10, "dexterity": 10, "intelligence": 10,
    "wisdom": 10, "constitution": 10, "agility": 10,
    "spell_power": 5, "magic_resist": 2,
    "resistances": {} # NEW: Add default resistances dictionary
}
PLAYER_BASE_XP_TO_LEVEL = 100
PLAYER_XP_TO_LEVEL_MULTIPLIER = 1.5
PLAYER_LEVEL_UP_STAT_INCREASE = 1
# The value a stat is assumed to hold when a player does not carry it. The
# neutral point of the shipped curves: a stat of 10 contributes nothing, so a
# missing stat is neutral rather than a penalty. Paired with
# `NPC_NEUTRAL_STAT_VALUE`, which is 8 because an NPC's stats sit lower.
PLAYER_NEUTRAL_STAT_VALUE = 10
PLAYER_MANA_LEVEL_UP_MULTIPLIER = ABILITY_POOL_LEVEL_UP_MULTIPLIER
PLAYER_MANA_LEVEL_UP_INT_DIVISOR = ABILITY_POOL_LEVEL_UP_STAT_DIVISOR
PLAYER_MANA_REGEN_WISDOM_DIVISOR = ABILITY_POOL_REGEN_STAT_DIVISOR
PLAYER_HEALTH_REGEN_STRENGTH_DIVISOR = 25
PLAYER_ATTACK_POWER_STR_DIVISOR = 3
PLAYER_DEFENSE_DEX_DIVISOR = 4
PLAYER_REGEN_TICK_INTERVAL = 1.0

# --- Player Combat ---
PLAYER_BASE_HIT_CHANCE = 0.85
PLAYER_ATTACK_DAMAGE_VARIATION_RANGE = (-1, 2)

# --- Player Status Display ---
PLAYER_STATUS_HEALTH_CRITICAL_THRESHOLD = 25
PLAYER_STATUS_HEALTH_LOW_THRESHOLD = 50
