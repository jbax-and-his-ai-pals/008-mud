...
class TestHeadlessEntitlementPolicyWiring(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self._tmp_data_root = repo_root / "server" / "tests" / "_tmp" / f"headless_topics_{uuid.uuid4().hex}"
        shutil.copytree(FANTASY_FRONTIER, self._tmp_data_root)
        self.addCleanup(lambda: shutil.rmtree(self._tmp_data_root, ignore_errors=True))
    ...
    def test_optional_topics_are_not_required_for_content_set_boot(self) -> None:
        server = HeadlessServer(db_path=":memory:", content_set_path=str(self._tmp_data_root))
        try:
    ...
    def test_boot_warning_policy_can_fail_server_start(self) -> None:
        shutil.rmtree(self._tmp_data_root / "data" / "magic")
        with self.assertRaises(RuntimeError):
            HeadlessServer(
                db_path=":memory:",
                content_set_path=str(self._tmp_data_root),
