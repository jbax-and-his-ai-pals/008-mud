...
class TestUIResolvedPlayerContext(GameTestBase):
    def setUp(self):
        super().setUp()
        fallback_player = Player("Fallback UI Hero", obj_id="fallback_ui_hero", world=self.world)
        ...
        # Simulate a state where no deprecated singleton is bound, but a resolved
