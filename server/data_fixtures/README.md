...
```powershell
python toolkit/editor_export_shim.py --source mud-world-editor/data --target tmp/editor_export_migrate_latest/content_data --report tmp/editor_export_migrate_latest/report.json --latest-root content_sets/fantasy_frontier/data --strict
```

Preferred refresh command (handles locked/read-only destination paths and writes `LATEST_REFRESH.json`):

```powershell
python toolkit/fixture_refresh.py --source mud-world-editor/data --latest-root content_sets/fantasy_frontier/data --fixture-root server/data_fixtures --fixture-name fantasy_editor_migrated_latest
```
