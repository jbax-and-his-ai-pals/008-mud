...
| Feature profile (`data/profiles/*.profile.json`) | `feature_profile.py` via `headless_server.py`; policy readers in `headless/party.py`, `shard.py`, `finite_adventure.py`, `world_effects.py` (provider ids) | `content_set.py::_validate_feature_profile` (added 2026-09-24; object-ness only before): the selected profile and its siblings — modes, provider ids only in custom mode, known sections and keys, policy values their readers act on (synonyms accepted), booleans, seconds, a real default campaign. `party.invite_conflict_policy` is read by nothing and refused | `FeatureProfileDialog.gd` (Explorer → Feature Profile…): a picker per mode, provider ids, every policy key typed; "(default)" leaves the key out; unknown values shown and kept; vocabulary parity-checked | journey-proven — switching combat off refuses a cast that killed (`m2_route_journeys_smoke.gd`) | `feature_profile_dialog_smoke.gd`, `schema_parity_smoke.gd`, `test_content_set_validator.py::TestFeatureProfile`, `feature_profile.py` | 6B |
...
| Room hazards (`properties.hazard_type`, `hazard_damage`, `hazard_tick_interval`, `weather_hazard_multipliers`) | `world/environment.py:59-106` | `content_set.py:431-587` — only when `data/combat/elements.json` exists; coverage only if the ruleset opts in | `RoomEnvironmentPanel.gd` picks declared hazards and offers contextual numeric/weather overrides; 0 means the declared value, shown beside it (until 2026-09-24 a 0.1 minimum on a 0.5 step turned a damage of 9 into 9.1 and a 5 s tick into 5.1) | journey-proven — an edited pit strikes twice as often for 20 (`m2_route_journeys_smoke.gd`, `edited_route_check.py hazard_exposure`). Damage is mitigated by defense/resistance, and five of fantasy's seven hazards did nothing at their region's level; retuned 2026-09-24, held by `test_fantasy_hazards_bite.py` | `save_engine_verdict_smoke.gd`, `room_environment_authoring_smoke.gd`, `content_set_runtime.py` (hazard damage), `environment_reader.py` | 6D |
...
| Region `spawner` (weights, toggles, level_range) | `spawner.py:104-171`; `region.py:68` | `content_set.py:412-428` (`level_range`); `content_set.py::_validate_region_spawners` — creatures, weights, toggles, unknown keys | `SpawnerInspector.gd` (removing or re-picking a creature errored and left the stale row on screen until 2026-09-24: its rows held a null refresh Callable) | journey-proven — a swamp edited to cave bears at level 4 spawns only those (`m2_route_journeys_smoke.gd`, `edited_route_check.py region_spawns`) | `spawner_inspector_smoke.gd`, `region_policy_validator.py` (partly), `save_engine_verdict_smoke.gd` (region saves roll back on refusal) | 6C |
...
| `properties.loot_tags` | `npc.py::_matches_ambient_loot_pool` (what the ruleset's ambient pools select on); `advancement.py::npc_payload` | `content_set.py::_npc_property_errors` — an array of non-empty strings (a bare string was dropped by the matcher, leaving the NPC out of every tag pool) | `NPCInspector.gd` "Loot tags" field, with a live line naming the pools the NPC falls into (or none) (added 2026-09-24; there was no control, and 30 of 41 fantasy hostiles had never been tagged) | validated writer | `npc_loot_tags_smoke.gd`, `test_content_set_validator.py::TestPatrolRoutes`, `test_fantasy_loot_ecology.py` (every fantasy hostile tagged; living ones roll a pool, the rest none) | 6D |
...
| `flavor_text` (and the retired `elemental_opposites`) | `magic/effects.py:123` (spell hits on a weakness/resistance; `default` indexed directly); `elemental_opposites` was loaded into a constant nothing read | `content_set.py::_validate_combat_flavor`: declared channels or `default`, the three known lines, only `{target_name}`, `default` required; `elemental_opposites` refused and removed from both sets (2026-09-24) | `CombatVocabularyDialog.gd` Hit Flavor section (one row per channel; `default` fixed) | validated writer | `combat_flavor_smoke.gd`, `test_content_set_validator.py` | 6D |
...
| `weather` (`profiles`, `descriptions`, `chances`) | `weather_manager.py:27-67`; `information.py:185` | `content_set.py::_validate_weather_shapes` checks `descriptions` and each profile's `map`/`travel_notes` are string maps, alongside the existing profile-reference check; `_validate_weather_chances` (added 2026-09-24): the four calendar seasons only, `summer` required (the engine looks it up on every roll, so a table without it raised `KeyError`), each season a table of weights of 0 or more with one above 0 (a bad weight quietly became "clear") | `RulesetEditorDialog.gd`: description rows plus one card per profile (id, map rows, travel-note rows); an authored empty `map`/`travel_notes` round-trips as empty rather than being dropped; `WeatherChancesSection.gd` (seasonal tables, with an off switch that shows the engine's own table, parity-checked) | journey-proven — a summer given its own table rolls clear or heatwave and the `weather` command describes it (`m2_route_journeys_smoke.gd`, `edited_route_check.py weather_rolls`) | `configuration_dialog_smoke.gd`; `ruleset_weather_smoke.gd`; `ruleset_weather_chances_smoke.gd`; `schema_parity_smoke.gd`; `test_content_set_validator.py`, `test_configuration_save.py` (staged engine verdict) | 6D |
...
| `crime` (+ `custody`) | `core/crime_manager.py:26-211`; `commands/jail.py:36-47` | `content_set.py::_validate_crime_and_debug_rules` — known keys, real numbers (the reader turns anything else into 0), boolean `enabled`, and while enabled a skill, reputation key and a `room_property` some room carries | yes — Crime (`CrimeSection.gd`); the jail-room check runs in the staged save | journey-proven — a theft caught for a fine ends in a cell once the jail threshold is lowered (`m2_route_journeys_smoke.gd`, `edited_route_check.py crime_theft`) | 6D |
...
| `economy.currency_name` | `world.py:155-157` | `content_set.py::_validate_simple_ruleset_sections` | yes — World Rules (`WorldRulesSection.gd`) | journey-proven — a renamed currency is what a fine is charged in (`m2_route_journeys_smoke.gd`, `crime_theft`) | 6D |
