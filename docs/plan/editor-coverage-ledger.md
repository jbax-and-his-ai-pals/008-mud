...
**Basis.** HEAD `93b7b9a` (2026-09-21), clean tree, plus two changes made while
building this ledger, both recorded in §I.6. Readers and validators were read out of
...
`run_content_checks.py` (29 steps), `run_editor_checks.py` (31 checks). Rows whose claim
...
| Room `env_properties` (incl. `weather`) | `room.py:21,30,141-142,190-199`; `weather_manager.py:50` | **none** | absent (no `env_properties` surface in the editor at all) | absent | none | 6C |
| Room hazards (`properties.hazard_type`, `hazard_damage`, `hazard_tick_interval`, `weather_hazard_multipliers`) | `world/environment.py:59-106` | `content_set.py:431-587` — only when `data/combat/elements.json` exists; coverage only if the ruleset opts in | declaration: `CombatVocabularyDialog.gd:47-85`; **placement: a free-form tag, no picker** (`CombatVocabularyCatalog.hazard_ids()` has no callers) | prototype | `configuration_dialog_smoke.gd`, `content_set_runtime.py` (hazard damage), `environment_reader.py` | 6D |
| Room `initial_npcs` (+ `overrides`) | `definition_loader.py:235-261` (whitelist `:245-250`) | `content_set.py:805-811` (not the whitelist) | `RoomContentPanel.gd:51-78` | prototype (extra keys invisible) | `content_round_trip_smoke.gd` | 6C |
| Room `items` (+ `properties_override`) | `definition_loader.py:230-234`; `world.py:624` | `content_set.py:812-816`; `reference_integrity_validator.py:143-156` | `RoomContentPanel.gd:51-78` | prototype (extra keys invisible) | `content_round_trip_smoke.gd` | 6C |
...
| NPC `faction` | `npc_factory.py:141`; `world/factions.py:155-176` | `content_set.py:883-926` — **warning only**; unknown faction silently becomes a bystander | absent (written as a monster-creation preset, `Main.gd:283`) | absent | `faction_dispositions.py` (synthetic set) | 6D |
| NPC `behavior_type` | `npc_factory.py:142`; `ai/dispatcher.py` | `content_set.py:909-915` — warning only; unknown value means the NPC never acts | absent | absent | `npc_behavior_vocabulary.py` | 6D |
| NPC `friendly` | `npc_factory.py:81` (overridden by `factions.is_hostile`) | **none** | absent | absent | none | 6D |
| NPC `dialog` (flat keyword dict) | `npc_factory.py:156-157`; `npc.py:85-86,124-125` | `template_placeholder_validator.py:97-134` (braces only) | absent | absent | `hostile_dialog_no_leftover_placeholders.py` | 6D |
...
| `loot_table` | `npc_factory.py:159-240`; `npc.py:160-174` | **none** | `NPCInspector.gd:124-203` — the id is a `Label`, so an item cannot be named | prototype | `content_round_trip_smoke.gd` | 6D |
...
| Containers (`contains`, `capacity`, `locked`, `key_id`, `is_open`) | `container.py:26-38, 339-351`; `item_factory.py:230-232` | **none** (item ids only) | class name only | absent | `container_authoring.py` (placement level) | 6C |
| Resource nodes (`resource_item_id`, `yield_table`, `substitute_resource_ids`, `charges`, `respawn_days`, `weather_blocked_by`) | `resource_node.py:16-30, 85-277`; `commands/gathering.py:68-74` | `content_set.py:2173-2231` — partial | class name only | absent | `resource_node_full.py`, `fantasy_gathering_route.py` | 6C |
...
| Stations (an item with `crafting_station_type`) | `crafting_manager.py` station lookup | via the recipe's station id | item property (scalar) only; **no station registry or picker** | prototype | `crafting_station.py` (synthetic) | 6C |
| `crafting.salvage_rules.by_family` / `default_item_id` | `crafting_manager.py:424-464` | `content_set.py:2083-2129` | absent — the Ruleset dialog has no `crafting` section | absent | `salvage_references.py` (synthetic) | 6C |
